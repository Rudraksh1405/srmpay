...
const normalizeAuthResponse = (response) => {
  if (response?.student) {
    return { token: response.token, user: { ...response.student, role: 'student' } };
  }
  if (response?.vendor) {
    const vendorId = response.vendor.id || response.vendor._id;
    return { token: response.token, user: { ...response.vendor, _id: vendorId, vendorId, role: 'vendor' } };
  }
  if (response?.admin) {
    return { token: response.token, user: { ...response.admin, role: 'admin' } };
  }
...
const authRequest = async (url, data) => {
  const response = await fetchReal(url, { method: 'POST', body: JSON.stringify(data) });
...
export const getVendorMenu = (vendorId) => fetchWithFallback(`/vendors/${vendorId}/menu`, {}, mockMenuItems[vendorId] || []);

// Orders - REAL ONLY
export const createOrder = (orderData) => fetchReal('/orders', { method: 'POST', body: JSON.stringify(orderData) });
export const getStudentOrders = (email) => fetchReal(`/orders/student/${email}`, {});
export const getVendorOrders = (vendorId) => fetchReal(`/orders/vendor/${vendorId}`, {});
export const updateOrderStatus = (orderId, status) => fetchReal(`/orders/${orderId}/status`, { method: 'PATCH', body: JSON.stringify({ status }) });

// Vendor Menu Update - REAL ONLY
export const updateMenuAvailability = (itemId, isAvailable) => fetchReal(`/vendors/menu/${itemId}`, { method: 'PATCH', body: JSON.stringify({ isAvailable }) });
...
export const updateVendorApproval = (id, approvalStatus) => fetchReal(`/admin/vendors/${id}/approval`, { method: 'PATCH', body: JSON.stringify({ approvalStatus }) });
